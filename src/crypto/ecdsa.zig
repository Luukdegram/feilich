...
pub const KeyPair = struct {
    ...
    pub fn init(maybe_seed: ?[seed_length]u8) !KeyPair {
        ...
        return KeyPair{
            ...
            .public_key = .{
                .x = affine.x.toBytes(.Little),
                .y = affine.y.toBytes(.Little),
            },
        };
    }
...
pub fn verify(public_key: KeyPair.PublicKey, msg: []const u8, signature: Signature) !bool {
    var digest: [Sha256.digest_length]u8 = undefined;
    Sha256.hash(msg, &digest, .{});
    const z = try Scalar.fromBytes(digest, .Big);

    const r = try signature.r();
    const s = try signature.s();
    ...
    return (try Scalar.fromBytes(x.toBytes(.Little), .Little)).equivalent(r);
}
